...
if(!empty($_POST['titreLieu']) && !empty($_POST['descriptionLieu']) && !empty($_POST['categorieLieu']) ) {
	...
	if(isset($_FILES) && !is_uploaded_file($tmp_file)) {
		...
		$response["STATUS"]="ERREUR_FICHIER";
		$response["MESSAGE"]="L'image ne peut pas être copiée, veuillez réessayer";
...
JSON;*/
		...
		$response["STATUS"]="ERREUR_TAILLE";
		$response["MESSAGE"]="L'image ajoutée est trop volumineuse (8Mo maximum)";
		echo json_encode($response);
		/*echo <<<JSON
...
JSON;*/
	...
	$response["STATUS"]="ERREUR_INCOMPLET";
	$response["MESSAGE"]="Informations manquantes dans le formulaire, veuillez les préciser";
	echo json_encode($response);
	/*
	echo <<<JSON
